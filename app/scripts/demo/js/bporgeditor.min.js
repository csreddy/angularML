/*
 Basic Primitives orgEditor Demo v2.0.0
...
null!=window.CodeMirror&&(this.dlgCodeMirror=this.element.find("[name=codemirror-form]").bpDlgCodeMirror());this.dlgItemConfig=this.element.find("[name=itemconfig-form]").bpDlgItemConfig();this.dlgMessage=this.element.find("[name=dialog-message]");this.dlgConfirm=this.element.find("[name=dialog-confirm]");this.orgDiagram=this.element.find("[name=orgchart-widget]");this.orgDiagram.css(a.getCSS());this.orgDiagram.orgDiagram();this.orgDiagram.orgDiagram("option",this._getOrgDiagramConfig());this.orgDiagram.orgDiagram("update");
this.buttonsPanel=this.element.find("[name=orgchart-panel]");this.buttonsPanel.css(c.getCSS());this.element.find("[name=options-button]").button().click(function(){b._onOptionsButtonClick()});this.element.find("[name=json-button]").button().click(function(){b._onJsonButtonClick()});this.element.find("[name=search]").autocomplete({source:this._getTitles(this.options.items)});this.element.find("[name=search]").on("change keyup",function(){var a=jQuery(this);a.data("last")!==a.val()&&(a.data("last",
a.val()),b._onChange(a.val()))})};primitives.orgeditor.Controller.prototype._getTitles=function(a){var c,b,d,e=[];c=0;for(b=a.length;c<b;c+=1)d=a[c],!0==d.isVisible&&!primitives.common.isNullOrEmpty(d.title)&&e.push(d.title);return e};
...
primitives.orgeditor.Controller.prototype._updateLayout=function(){var a=new primitives.common.Rect(0,this.panelHeight,this.element.innerWidth(),this.element.innerHeight()-this.panelHeight),c=new primitives.common.Rect(0,0,a.width,this.panelHeight);this.formsPlaceholder.css(a.getCSS());this.orgDiagram.css(a.getCSS());this.buttonsPanel.css(c.getCSS())};primitives.orgeditor.Controller.prototype.update=function(a){this._updateLayout();this.orgDiagram.orgDiagram("update",a)};
